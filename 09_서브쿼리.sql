...
-- 다음 문장은 서브쿼리가 리턴하는 행이 여러 개라서 단일행 연산자를 사용 할 수 없습니다.
-- 단일 행 연산자: 주로 비교 연산자(=, >, <, >=, <=, <>)를 사용하는 경우 하나의 행만 반환해야 합니다.
-- 이런경우에는 다중 행 연산자를 사용해야 합니다.
SELECT * FROM employees
WHERE job_id = (SELECT job_id FROM employees
                WHERE job_id = 'IT_PROG'); -- 에러
                
-- 다중 행 연산자 (IN, ANY, ALL)
-- IN: 조회된 목록의 어떤 값과 같은 자를 확인합니다.
SELECT * FROM employees
...
-- EXIST: 서브쿼리가 하나 이상의 행을 반환하면 창으로 간주.
-- job.history에 존재하는 직원이 employees에도 존재한다면 조회에 포함
