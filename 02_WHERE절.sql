...
SELECT * FROM employees
WHERE job_id IN ('IT_PORG', 'AD_VP');

-- LIKE 연산자
-- %는 어떠한 문자든, _는 데이터의 자리(위치)를 찾아낼 때
SELECT first_name, last_name, hire_date
FROM employees
WHERE hire_date LIKE '03%'; -- 앞에03

SELECT first_name, hire_date
FROM employees
WHERE hire_date LIKE '%15'; -- 뒤에15

SELECT first_name, hire_date
FROM employees
WHERE hire_date LIKE '%05%';-- 어디든 05

SELECT first_name, hire_date
FROM employees
WHERE hire_date LIKE '___05%'; --_ 문자하나당 >>4번째숫자가0 5
